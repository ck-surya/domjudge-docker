...
if ! bin/dj_setup_database -u"${MYSQL_ROOT_USER}" -p"${MYSQL_ROOT_PASSWORD}" status > /dev/null 2>&1
then
	...
	echo "Using ${INSTALL}..."
	bin/dj_setup_database -u"${MYSQL_ROOT_USER}" -p"${MYSQL_ROOT_PASSWORD}" ${INSTALL}
else
	...
	bin/dj_setup_database -u"${MYSQL_ROOT_USER}" -p"${MYSQL_ROOT_PASSWORD}" upgrade
fi
